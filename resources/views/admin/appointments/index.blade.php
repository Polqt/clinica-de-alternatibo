    ...
    <div class="mb-6">
        <div class="border-b border-slate-200 dark:border-slate-700">
            <nav class="flex -mb-px overflow-x-auto">
                ...
                <a href="{{ route('admin.appointments', ['status' => 'rescheduled']) }}" class="{{ request('status') == 'rescheduled' ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300' }} py-3 px-4 text-sm font-medium">
                    Rescheduled
                    <span class="ml-1 bg-purple-100 dark:bg-purple-900/40 text-purple-800 dark:text-purple-300 py-0.5 px-2 rounded-full text-xs">
                ...
                <a href="{{ route('admin.appointments', ['status' => 'cancelled']) }}" class="{{ request('status') == 'cancelled' ? 'text-blue-600 dark:text-blue-400 border-b-2 border-blue-600 dark:border-blue-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300' }} py-3 px-4 text-sm font-medium">
                    ...
                    <span class="ml-1 bg-red-100 dark:bg-red-900/40 text-red-800 dark:text-red-300 py-0.5 px-2 rounded-full text-xs">
                        {{ $statusCounts['cancelled'] ?? ($statusCounts['cancelled_patient'] ?? 0) + ($statusCounts['cancelled_clinic'] ?? 0) }}
                    </span>
                </a>
            </nav>
        </div>
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between">
                    <div class="flex items-start space-x-4">
                        ...
                        <div>
                            ...
                            @if($appointment->status === 'rescheduled' && $appointment->reschedule_reason)
                            <div class="mt-2 flex items-center text-sm text-purple-600 dark:text-purple-400">
                                <flux:icon name="info-circle" class="w-4 h-4 mr-1" />
                                <span>Rescheduled: {{ $appointment->reschedule_reason }}</span>
                            </div>
                            @endif
                        </div>
                    </div>
